...
Original file is located at
    https://colab.research.google.com/drive/1F-2b4SdkOyTSb3CSTTDETNVKnvMa_kiP
...
def compute_FastText_embeddings(pd_dataframe, embedding_dim):
   ...
   # vocab_size = len(sentences.word2idx)
  

    model = FastText(size = embedding_dim)

    vocab=[]
    vocab = [i for item in sentences for i in item]
    vocab_size = len(vocab)
    # print(vocab[0:5])
    model.build_vocab(sentences=sentences)

    model.train(vector_size=embedding_dim, sentences=sentences, total_examples=len(sentences), epochs=10)

    embedding_vec = np.zeros([2+vocab_size, embedding_dim])

    embedding_vec = model.wv.vectors

    x = embedding_vec
    #x = np.insert(x,0,np.zeros([1, embedding_dim]))
    x= np.append(x, np.zeros([1, embedding_dim]), axis=0)
    x= np.append(x, np.zeros([1, embedding_dim]), axis=0)
...
'''
    ...
    for i in range(len(vocab)):
        temp = [word_to_index[w] for w in vocab[i]]
...
class RnnEncoder(nn.Module):
    def __init__(self, pretrained_emb, vocab_size, embedding_dim, hidden_units):
        ...
        #torch_pretrained_emb = torch.from_numpy(pretrained_emb.values)

        # Initialize embedding layer with pretrained_emb
        # (see: https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html)
        torch_pretrained_emb = torch.FloatTensor(pretrained_emb)

        self.embedding = nn.Embedding.from_pretrained(torch_pretrained_emb)
        
        # Initialize a single directional GRU with 1 layer and batch_first=False
        #self.gru = nn.GRU(hidden_units, hidden_units, num_layers = embedding_dim, batch_first=False)
        #self.gru = nn.GRU(hidden_units, hidden_units, batch_first=False)
...
class RnnDecoder(nn.Module):
    def __init__(self, pretrained_emb, vocab_size, embedding_dim, hidden_units):
        ...
        self.va = nn.Linear(hidden_units, 1, bias=True)
        self.w1 = nn.Linear(hidden_units, hidden_units, bias=True)
        self.w2 = nn.Linear(hidden_units, hidden_units, bias=True)
        ...
        self.rnn = nn.GRU(embedding_dim + hidden_units, hidden_units, num_layers=1, batch_first=True)
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        score = self.va(torch.tanh(self.w1(dec_hs) + self.w2(enc_output)))  # [batch_size, max_len, 1]
        #score = score.squeeze(2).unsqueeze(1) 
        attention_weights = F.softmax(score, dim=1) # [batch_size, 1, max_len]
        #attention_weights = F.softmax(score) # [batch_size, 1, max_len]
      #  context_vector = torch.bmm(attention_weights, enc_output) # [batch_size, 1, enc_units]
       ...
       # attention_vector = torch.tanh(self.wc(torch.cat((context_vector, dec_hs), dim=2))) # [batch_size, 1, dec_units]
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        '''
        #fc_out, dec_hs, attention_weights = None, None, None

        ### TODO ###
        context_vector, attention_weights = self.compute_attention(dec_hs, enc_output)
        x_embedding = self.embedding(x)

       # input = torch.cat((context_vector, x_embedding.float()), dim=1) 
        input = torch.cat((context_vector.unsqueeze(1), x_embedding.float()), dim=2) 
        output, dec_hs = self.rnn(input, dec_hs)

        fc_out = self.fc(output.squeeze(1))

        #print('x: ',x_embedding.shape)
...
class TransformerEncoder(nn.Module):
    def __init__(self, pretrained_emb, src_vocab_size, embedding_dim, num_heads,
        ...
        #self.register_buffer('position_embedding', self.position_embedding) # this informs the model that position_embedding is not a learnable parameter
        ...
        te = nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=dim_feedforward)
        self.transformer_encoder = nn.TransformerEncoder(te,num_layers)
    ...
    def forward(self, x):
       ...
       # mask = mask.to(self.device)
        ...
        x = self.position_embedding + embedded
        x = self.dropout(x)
...
class TransformerDecoder(nn.Module):
    def __init__(self, pretrained_emb, trg_vocab_size, embedding_dim, num_heads,
        ...
        # num_layers, num_heads, & dim_feedforward here)
        td = nn.TransformerDecoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=dim_feedforward)
        self.transformer_decoder = nn.TransformerDecoder(td,num_layers)
    ...
    def forward(self, dec_in, enc_out):
        ...
        ### TODO ###
        #mask_enc = enc_out.shape[0]
        mask_enc = dec_in.shape[0]
        mask = self.generate_square_subsequent_mask(mask_enc).to(self.device)

        embedded = self.embedding(dec_in) 

        x_shape = embedded.shape

        dpe = self.position_embedding

        #dpe = tf.reshape( dpe, dec_in.shape, name=None)
        #print(dpe.shape)
        #print(dec_in.shape)

        #dpe = dpe[1:].reshape(-1)
        #dpe = dpe.reshape(-1, dpet.shape[2])

        dpe = dpe.resize_(x_shape)
        #print('dpe:', dpe.shape)

        #dpe = dpe.reshape(-1, dpe.shape[2])

        dpe = embedded + dpe
        dpe = self.dropout(dpe)

        #mask = self.generate_square_subsequent_mask(x)
        output = self.transformer_decoder(dpe, enc_out, mask)
         #output = self.transformer_encoder(x, src_key_padding_mask = mask)
